       ...
       <nav class="level">
        ...
        <b-table :data="usersList" :paginated="true" :per-page="perPage" :current-page.sync="currentPage" :loading="loadData" :pagination-simple="true" :narrowed="true" :mobile-cards="true" :striped="true" :hoverable="true" :default-sort-direction="defaultSortDirection" default-sort="created_at"
            ...
            <template slot-scope="props">
                ...
                <b-table-column field="cs.name" label="Cs" sortable >{{ props.row.cs.name }}</b-table-column>
                <b-table-column field="cashierr.name" label="Kasir" sortable >{{ props.row.cashier.name }}</b-table-column>
            ...
            <template slot="detail" slot-scope="props">
                    <div class="columns" v-for="dt in props.row.detail" :key="dt.id" style="border-bottom: solid 1px black">
                        <div class="column">
                            Montir <br> <span v-if="dt.montir != null">{{dt.montir.name}}</span> <span v-else>-</span>
                        </div>
                        <div class="column">
                            Plat Kendaraan <br> {{dt.vehicle_customer.licensePlate}}
                        </div>
                        <div class="column">
                            Sparepart <br>
                            <ul>
                                <li v-for="sp in dt.detail_transaction_sparepart" :key="sp.id">{{sp.sparepart.name}}({{sp.total}}) biaya: {{sp.total * sp.price}}</li>
                            </ul>
                        </div>
                        <div class="column">
                            Layanan <br>
                            <ul>
                                <li v-for="sv in dt.detail_transaction_service" :key="sv.id">{{sv.service.name}}({{sv.total}}) biaya: {{sv.total * sv.price}}</li>
                            </ul>
                        </div>
                    </div>
            </template>
